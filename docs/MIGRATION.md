...
| P3 | Replace `gulp-sharp-responsive` (private git fork) with an actively maintained responsive-image tool. **Re-prioritized 2026-08-11**: executing F1 revealed the tool is currently broken under `gulp@5.0.1` (`generate-responsive-images` hangs, writes nothing to `dist/`) and carries a nested, unfixable `sharp@0.33.5` CVE regardless of this repo's own lockfile — no longer a someday item, treat as near-Foundation priority. See [docs/specs/09](specs/09-replace-responsive-image-tool.md). | F1 | Medium |
