...
| ID | Title | Depends on | Complexity |
|---|---|---|---|
| P1 | SCSS `@import` → `@use` migration. **In progress 2026-08-11**: `email.scss` converted and verified (byte-identical output); `core/variables/_colors.scss` + `core/_colors.scss` merged into one file per a mid-execution simplification request, verified via sorted-diff (identical rules, one safe reorder). Proceeding one aggregator at a time, each independently verified — see the progress log in [docs/specs/07](specs/07-scss-import-to-use-migration.md). | F5 | Medium |
