...
| ID | Title | Depends on | Complexity |
|---|---|---|---|
| P1 | SCSS `@import` → `@use` migration. **In progress 2026-08-11**: `email.scss`, the will-style-only portion of `_core.scss`, and the will-style-only portion of `_site.scss` converted and verified content-identical. Two hard rules found and documented in [docs/standards.md](standards.md): (1) a module with real CSS output can't be safely both `@import`'d and `@use`'d in the same compile — fixed via the `core/_colors.scss`/`elements/_colors.scss` split pattern; (2) `@extend` across a `@use` boundary needs a same-context module relationship, which can itself retrigger rule (1) — resolved in `elements/_tabs.scss` by inlining instead of extending. Also: namespace collisions get fixed by renaming the file, not aliasing the `@use` (now standard practice). Bootstrap-coupled parts of both `_core.scss` and `_site.scss` deliberately left on `@import` — converting those needs Bootstrap's own `@use ... with()` pattern, a separate larger redesign. `components/forms` also deferred (extends Bootstrap classes directly). `_app.scss`, `_libraries.scss` still pending — see the progress log in [docs/specs/07](specs/07-scss-import-to-use-migration.md). | F5 | Medium |
