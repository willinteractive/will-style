...
- Any SCSS file (will-style's own or a wrapper like `_app.scss`/`_libraries.scss`) that reaches Bootstrap-owned variables through the shared `@import` global scope stays on `@import` until Bootstrap's own source ships `@use`/`@forward` support — confirmed 2026-08-11 by reading the vendored `bootstrap` gem's SCSS directly (5.3.8: 0 files use `@use`, 8 still use `@import`). This isn't a will-style gap to close with more effort; there's no module-system surface on the Bootstrap side to convert onto yet. Don't attempt to "finish" this by converting a pass-through wrapper file alone — the wrapper compiling without error doesn't mean the bare Bootstrap variables inside it still resolve correctly, since `@use` walls off a module's scope from the outer `@import` world (see [docs/specs/07](specs/07-scss-import-to-use-migration.md)'s Outcome section). Re-check this the next time the `bootstrap` gem is upgraded — a future major may add module-system support.
