...
3. **`access` and `learning` are on Rails `6.1.7.x`**, not Rails 8. **Resolved 2026-08-11**: the team confirmed both are being modernized to Rails 8 next, so `will_style.gemspec`'s `rails` floor was raised to `>= 8.0` (see [dependency-audit.md](../dependency-audit.md)) as a deliberate forcing function — those two apps simply can't pick up a new will-style release until their own Rails/Ruby upgrade lands, which is the intended order of operations.
