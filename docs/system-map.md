...
All 21 imported files are still plain `(function() { 'use strict'; ... })();` IIFEs (zero `import`/`export` syntax internally) attaching to a global `window.WillStyle` namespace, first established in [`core/settings.js`](../app/javascript/will_style/core/settings.js) and [`core/events.js`](../app/javascript/will_style/core/events.js). `settings.js` defines `window.WillStyle.Settings.pageChangeEvent = "turbo:load"`, which the feature/component/form scripts key their re-initialization off of. Correctness still depends on load order (now encoded as explicit `import` statement order in `will_style.js` rather than `//= require` order) and pre-existing globals — this load-order/global-namespace coupling is what item C2 (real ES modules with explicit `import`/`export`) eventually removes; C1 only changed the loading *mechanism*, not the files' internals.
