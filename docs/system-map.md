...
**JavaScript — single loading system as of 2026-08-11 (item C1)**: [`config/importmap.rb`](../config/importmap.rb) (2 lines): `pin_all_from "../app/javascript/will_style", under: "will_style"` pins each of the 21 files under `app/javascript/will_style/` individually, and `pin "will_style", to: "will_style.js", preload: true` pins the aggregate entry point, [`app/javascript/will_style.js`](../app/javascript/will_style.js) — wired into the host app via the `will_style.importmap` initializer above. That entry point used to be a Sprockets manifest (`//= require` directives); it's now a plain ESM file of side-effect `import` statements against the `pin_all_from` names, same load order (vendor → core → features → components → forms), same public `import "will_style"` contract for consuming apps. No more Sprockets-directive dependency.

As of item C2 (2026-08-11), all 20 imported behavior files (the 21st, [`vendor/growfield.js`](../app/javascript/will_style/vendor/growfield.js), is third-party UMD, left as-is) are real ES modules — IIFE wrappers removed, `window.WillStyle.*` reads replaced with explicit `import`s from [`core/settings.js`](../app/javascript/will_style/core/settings.js) (`Settings`) and [`core/events.js`](../app/javascript/will_style/core/events.js) (`trigger`/`on`). Execution order is now the explicit `import` statement order in `will_style.js` rather than implicit global load order. Three globals remain as **deliberate compatibility shims**, not leftovers: `window.WillStyle.Settings`, `.Events`, and `.Forms.initializeExpandingTextareas`. `.Events` is load-bearing — [`_deferred_styles.html.erb`](../app/views/will_style/components/_deferred_styles.html.erb) has an inline `<script>` outside the module graph that polls `window.WillStyle.Events` directly to fire `css-initialized` as soon as the stylesheet preload resolves. `.Settings`/`.Forms` have no confirmed reader in this repo, but nothing rules out a consuming app's own JS depending on them, so they were kept rather than guessed away.
